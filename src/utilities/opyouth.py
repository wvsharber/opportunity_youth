"""
This .py file is used to explore the updated data of the Opportunity Youth. 
It creates a report, which can be seen in our Jupyter Notebook that accompanies 
this file.
...
def pullsqldata():
    ...
    ## create the sql engine, write the query, and extract the query into a pd
    ## DF
    engine = create_engine("postgresql:///opportunity_youth")
    queries = [
        ("""SELECT * FROM oyyouth_from_pumas"""),
    ...
    poptotals["year"] = 2017
    OYtotals["year"] = 2017
    pivot_total_youth_pop = total_population.pivot(
        index="oy_flag", columns="age_flag", values=["estimate", "total", "percent"]
    )
    ...
    pivot_opportunity_youth = opportunity_youth.pivot(
        index="edu_flag", columns="age_flag", values=["estimate", "total", "percent"]
    )
    pivot_opportunity_youth.to_csv(
        "../../data/processed/opportunity_youth_education_2017"
    )
    ...
    educationtable2014 = pd.read_csv(
        "../../data/processed/2016educationtable - Sheet1.csv"
    )
    educationtabletotals2014 = pd.read_csv(
        "../../data/processed/2016educationtable_totals - Sheet1.csv"
    )
    opyouthtable2014 = pd.read_csv(
        "../../data/processed/2016opportunityyouthtable - Sheet1.csv"
    )
    opyouthtabletotals2014 = pd.read_csv(
        "../../data/processed/2016opportunityyouthtable_totals - Sheet1.csv"
    )
    ...
    youth_population_2014_2017 = pd.concat(
        [total_population, opyouthtable2014], ignore_index=True
    )
    youth_population_totals_2014_2017 = pd.concat(
        [poptotals, opyouthtabletotals2014], ignore_index=True
    )
    oy_population_2014_2017 = pd.concat(
        [opportunity_youth, educationtable2014], ignore_index=True
    )
    oy_population_totals_2014_2017 = pd.concat(
        [OYtotals, educationtabletotals2014], ignore_index=True
    )
    return (
        opp_youth,
        total_population,
...
def plotdata1(df):
    ...
    sns.barplot(x="year", y="total_population", data=df)
    plt.title("Total Population of Youth \n Ages 16-24 in South King Co.")
    plt.ylabel("Population")
    plt.yticks(fontsize="x-small")
    plt.xlabel("Year")
    plt.tight_layout()
    plt.savefig("../../reports/figures/01_totalpop.png", dpi=300)
    return fig, ax
...
def plotdata2(df):
    ...
    sns.barplot(x="year", y="percent", data=df.loc[df["oy_flag"] == "OY"])
    plt.title(
        "Percent of Opportunity Youth in \n South King Co. between 2014 and 2017",
        wrap=True,
    )
    ...
    plt.xlabel("Year")
    plt.tight_layout()
    plt.savefig("../../reports/figures/02_percentOY.png", dpi=300)
    return fig, ax
...
def plotdata3(df):
    ...
    plt.tight_layout()
    plt.savefig("../../reports/figures/03_YouthPopulationbyAge.png", dpi=300)
...
def plotdata4(df):
    ...
    plt.tight_layout()
    plt.savefig("../../reports/figures/04_OppYouthPopulationbyAge.png", dpi=300)
    return fig, ax1
...
def plotdata5(df):
    ...
    sns.barplot(
        x="year", y="percent", data=df.loc[df["edu_flag"] == "No Diploma"], ax=ax1
    )
    ...
    plt.tight_layout()
    plt.savefig("../../reports/figures/05_OpportunityYouthEducation.png", dpi=300)
...
def make_locator_map():
    ...
    """This function produces a live folium map to display where s. king county
    is in the greater seattle area"""
    ...
    sm = folium.Map(
        [47.5, -122.3502], zoom_start=9.4, crs="EPSG3857", zoom_control=False
    )

    folium.GeoJson(s_king_df).add_to(sm)
    sm.save("../../reports/figures/06_S_King_County_Locator_Map.html")
    return sm


def prepare_df_for_choropleth(df1, df2):

    """ Ths function engineers the features into the dataframe to plot the choropleth map"""
    ...
    s_king_df = wa_puma.loc[wa_puma["king_county"] == 2]
    s_king_df.index = s_king_df[
        "PUMACE10"
    ]  # reset the s_king_df index to be the puma id
    OYbyPUMA_df = pd.DataFrame(
        OYbyPUMA
    )  # create the dataframe containing the estimated # of OY
    ...
    s_king_df["OY_percent"] = round(
        s_king_df["pwgtp"] / s_king_df["total_youth_pop"] * 100
    )
...
def make_choropleth_OYpop_map(geo_df):
    ...
    geo_df.plot(
        column="pwgtp", ax=ax, cmap="YlOrBr", legend=True, cax=cax, edgecolor="black"
    )

    plt.savefig("../../reports/figures/07_OY_by_puma_map.png", dpi=200)
    return fig, ax
